...
// is the right primitive — it can reach every endpoint in the catalog without this file
...
//   types                 key -> "boolean" | "string" | "number" | "array" |
//                         "object" | "null", or an array of those when the
//                         endpoint genuinely accepts more than one
//   arrayLimits           key -> the most entries the endpoint accepts, for the
//                         bulk endpoints that 400 on an oversized list
...
//   requiredWhen           conditional requirement keyed on another key's value:
//                          [{ when: { key, equals? , in? }, nestedRequired: { key,
//                          required }, message }]. Lets a contract say "if
//                          `type` is Reference, `metadata.collectionId` is
//                          required" instead of leaving that as prose in `note`
//                          — the difference between documenting a mistake and
//                          refusing it before it reaches a live site.
...
export const BODY_CONTRACTS = {
  ...
  // The --live form of `wf items set` PATCHes this endpoint instead of
  // items/update-item. No `isDraft` here: a live item is already published, so
  // "draft" does not apply to it — sending it would be a stray key on an
  // endpoint whose real shape is not fully published by Webflow, so it is left
  // out rather than guessed at.
  "items/update-item-live": {
    bodyRequired: true,
    optional: ["fieldData", "isArchived", "cmsLocaleId"],
    types: { fieldData: "object", isArchived: "boolean" },
    strayKeysBelongIn: "fieldData",
    nested: {
      fieldData: { note: "Only the keys you send change. This writes the LIVE (published) item directly — there is no staging step to review first." }
    },
    example: { fieldData: { name: "Acme Ltd" } },
    note: "Bypasses the normal staged → publish flow. Prefer `wf items set` without --live, then publish, unless you specifically need to edit the published copy directly.",
    source: "docs"
  },
  "items/publish-item": {
    bodyRequired: true,
  ...
  "fields/create": {
    ...
    // The historical failure this closes: "Reference fields must have a
    // collectionId" used to live only as prose in the `note` above — true, but
    // it cost a live 400 to learn. Same shape for Option/`metadata.options`.
    // `wf fields add` builds these from --to/--options so the mistake can't
    // happen from that command either.
    requiredWhen: [
      {
        when: { key: "type", in: ["Reference", "MultiReference"] },
        nestedRequired: { key: "metadata", required: ["collectionId"] },
        message:
          'type "Reference"/"MultiReference" needs metadata.collectionId — pass it directly, or use `wf fields add <collectionId> --type Reference --name <name> --to <targetCollectionId>`.'
      },
      {
        when: { key: "type", equals: "Option" },
        nestedRequired: { key: "metadata", required: ["options"] },
        message:
          'type "Option" needs metadata.options (its choices) — pass it directly, or use `wf fields add <collectionId> --type Option --name <name> --options a,b,c`.'
      }
    ],
  ...
  // ── Page schema markup (JSON-LD), beta ─────────────────────────────────
  // `wf page-schema` assembles exactly these bodies (bin/wf.mjs), so the
  // contract and the typed command check each other in the test suite. The
  // shapes themselves come from the beta reference, hence source: "docs".
  "pages/update-schema-markup": {
    bodyRequired: true,
    required: ["jsonLdSchema"],
    types: { jsonLdSchema: ["object", "string", "null"] },
    example: { jsonLdSchema: { "@context": "https://schema.org", "@type": "FAQPage", mainEntity: [] } },
    note: 'PUT replaces the page\'s whole JSON-LD block — there is no partial update. `null` clears it. A string is accepted too: raw JSON, or JSON wrapped in <script type="application/ld+json">…</script> (an opening tag without its closing tag is a 400). Target a secondary locale with `--q localeId=…`; without one you are writing the primary locale. Limits per entry: 60KB raw, 32 levels of nesting, 5000 nodes. `wf page-schema set` builds this body for you.',
    source: "docs"
  },
  "pages/query-schema-markup-bulk": {
    bodyRequired: true,
    required: ["pages"],
    types: { pages: "array" },
    arrayLimits: { pages: 100 },
    nested: {
      pages: {
        note: "Each entry is `{ id }`, plus an optional `localeId`. The same page id may repeat only with a different localeId — a duplicate (id, localeId) pair is a 400."
      }
    },
    example: { pages: [{ id: "dddddddddddddddddddddddd" }] },
    note: "The read that a site-scoped grant can actually verify: the site id is in the path, where the single-page GET carries only a page id. Up to 100 entries. `wf page-schema --site <id> --pages <ids>` builds this body for you.",
    source: "docs"
  },
  "pages/update-schema-markup-bulk": {
    bodyRequired: true,
    required: ["pages"],
    types: { pages: "array" },
    arrayLimits: { pages: 25 },
    nested: {
      pages: {
        note: "Each entry is `{ id, jsonLdSchema }`, plus an optional `localeId`. `jsonLdSchema` is required per entry and takes the same object/string/null values as the single-page PUT."
      }
    },
    example: { pages: [{ id: "dddddddddddddddddddddddd", jsonLdSchema: null }] },
    note: "Replaces the JSON-LD for every listed page — same all-or-nothing semantics as the single-page PUT, 25 entries max. `wf page-schema set --site <id>` builds this body for you.",
    source: "docs"
  },
...
export const validateBody = ({ contract, body, method = "POST" }) => {
  ...
  for (const [key, want] of Object.entries(contract.types || {})) {
    ...
    if (!allowed.includes(got)) errors.push(`"${key}" must be ${allowed.join(" or ")}, not ${got}.`);
  }

  // A bulk list longer than the endpoint accepts is a 400 for the whole
  // request, so every entry in it is wasted along with the grant call.
  for (const [key, max] of Object.entries(contract.arrayLimits || {})) {
    const value = body[key];
    if (Array.isArray(value) && value.length > max) errors.push(`"${key}" takes at most ${max} entries, not ${value.length}. Split the work into batches.`);
    if (Array.isArray(value) && value.length === 0) errors.push(`"${key}" is empty — the endpoint needs at least one entry.`);
  }
  ...
  // requiredWhen: a value on one key makes a nested key required on another —
  // e.g. type "Reference" requires metadata.collectionId. Only fires when the
  // trigger key is actually present in this body; a rule never invents a
  // requirement for a key the caller did not send.
  for (const rule of contract.requiredWhen || []) {
    const triggerValue = body[rule.when.key];
    if (triggerValue === undefined) continue;
    const matches = "equals" in rule.when ? triggerValue === rule.when.equals : Array.isArray(rule.when.in) && rule.when.in.includes(triggerValue);
    if (!matches) continue;
    const nestedValue = body[rule.nestedRequired.key];
    const hasAll = typeOf(nestedValue) === "object" && (rule.nestedRequired.required || []).every((req) => req in nestedValue);
    if (!hasAll) errors.push(rule.message);
  }
...
export const renderContract = ({ endpoint, contract }) => {
  ...
  for (const [key, want] of Object.entries(contract.types || {})) {
    if (Array.isArray(want)) lines.push(`  ${key}: ${want.join(" or ")}`);
  }
  for (const [key, max] of Object.entries(contract.arrayLimits || {})) {
    lines.push(`  ${key}: 1 to ${max} entries (checked locally — an oversized list is a 400 for the whole batch)`);
  }
  for (const [key, spec] of Object.entries(contract.nested || {})) {
