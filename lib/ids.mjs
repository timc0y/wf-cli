...
// Resource-id routes do not carry the owning site in their URL. A site-scoped
// grant or project pin must refuse them instead of treating the missing site
// as a match, unless a local resource-to-site cache exists to check against.
// Collections and pages are excluded from that default refusal for exactly
// this reason: their collection->site and page->site caches (grants.mjs) are
// proven ownership lookups. The other 4 prefixes here still have no cache, so
// they still fail closed.
