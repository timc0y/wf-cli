...
export const cacheSites = (profile, sites) => {
  setProfileMeta(profile, {
    sitesCache: {
      ...
      sites: (sites || []).map((s) => ({ id: s.id, displayName: s.displayName || null, shortName: s.shortName || null }))
    }
  });
};

export const getCachedSites = (profile) => listProfiles()[profile]?.sitesCache || null;
...
export const cacheCollections = (profile, siteId, collections) => {
  const profiles = listProfiles();
  if (!profiles[profile]) return;
  const existing = profiles[profile].collectionsCache || {};
  const now = new Date().toISOString();
  for (const c of collections || []) {
    if (c?.id) existing[c.id] = { siteId, cachedAt: now };
  }
  setProfileMeta(profile, { collectionsCache: existing });
};

export const getCachedCollectionSite = (profile, collectionId) => listProfiles()[profile]?.collectionsCache?.[collectionId]?.siteId || null;
