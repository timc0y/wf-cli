...
const HOST = "https://api.webflow.com";
const BASE = `${HOST}/v2`;

// Beta endpoints hang off their own version root — `/beta/…`, NOT `/v2/beta/…`
// (the beta OpenAPI spec declares that single server, and its generated code
// samples use it; one changelog entry says `/v2/beta/` and is the odd one out).
// A catalog path that starts with `beta/` is therefore joined to the host, not
// to BASE. Everything downstream is unchanged: the path is still classified
// with the beta prefix stripped (lib/ids.mjs), so the grant site-scope and the
// project pin treat it as the ordinary resource it is.
const baseForPath = (clean) => (/^beta(\/|$)/i.test(clean) ? HOST : BASE);
...
const buildUrl = (path, query) => {
  ...
  const url = new URL(`${baseForPath(clean)}/${clean}`);
...
export const webflowRequest = async ({
  ...
  if (dryRun) {
    ...
    const confirmTarget = confirmationTargetFor(method, path, body);
    return {
      ok: true,
      status: 0,
      dryRun: true,
  ...
  // target: --confirm <id> has to match the id in the resolved path — or, for
  // bulk item publish, the whole set of ids in the body (see grants.mjs). This
  // forces the caller to name what it is about to destroy or publish — a
  // copy-pasted or template-substituted wrong target no longer sails through.
  const confirmTarget = confirmationTargetFor(method, path, body);
...
// GET /sites/{id}/pages is ALSO grant-free, for the same circularity reason
// as listCollectionsFree — pages/{page_id} endpoints (get-metadata,
// update-page-settings, ...) never carry a site id in their own URL, so
// grants.mjs can't check them without this cache (see profiles.mjs's
// cachePages). Read-only, exposes page titles/slugs/SEO fields (not element
// content), and still requires an already-stored token for the profile — not
// universally free, just exempt from the grant/tier/scope/budget machinery
// like listSitesFree. Note: a single call, same as listCollectionsFree — a
// site with more pages than the API's default page size will need `wf pages
// refresh` re-run, or this helper extended to page through the results, if
// that ever bites in practice.
export const listPagesFree = async (profile, siteId) => {
  return freeListRequest({ profile, siteId, path: `sites/${siteId}/pages`, resultKey: "pages" });
};
