...
import { getCachedCollectionSite, getCachedPageSite, validateProfileName } from "./profiles.mjs";
...
const authorizeUnlocked = ({ profile, method, path, group = null }) => {
  ...
  if (grant.siteIds) {
    ...
    if (siteMatch) {
      ...
      // pages/{page_id} is the one resource in resourceIdInPath's fail-closed
      // set with a proven ownership lookup: the page -> site cache just
      // below, the same fix as collectionIdInPath's cache a few lines down.
      // The other 4 prefixes in SITE_OWNED_RESOURCE_PREFIXES (assets,
      // asset_folders, form_submissions, forms) still fail closed; nobody
      // has built their cache yet.
      if (resourceMatch && !resourceMatch.invalid && resourceMatch.resource === "pages") {
        const knownSite = getCachedPageSite(name, resourceMatch.id);
        if (!knownSite) {
          return {
            ok: false,
            code: CODES.WF_GRANT_SCOPE,
            error: `Grant for "${profile}" is site-scoped, but page ${resourceMatch.id} isn't in the site-scoping cache — it can't be verified as belonging to an allowed site.`,
            hint: `Run \`wf pages refresh --sites ${grant.siteIds.join(",")}\` (free, no grant needed) to populate the cache, then retry. If the page was created after this grant was issued, that's the likely cause.`
          };
        }
        if (!grant.siteIds.includes(knownSite)) {
          return {
            ok: false,
            code: CODES.WF_GRANT_SCOPE,
            error: `Grant for "${profile}" is scoped to site(s) [${grant.siteIds.join(", ")}] but ${method} ${path} targets page ${resourceMatch.id}, which belongs to a different site (${knownSite}).`,
